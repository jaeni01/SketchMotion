...
| **Confluence** | ⚠️ 보류 (API 권한 없음) | 이 세션의 Atlassian 커넥터에 Jira 스코프(read/write:jira-work)만 있고 Confluence 스코프가 없어 API 접근 불가(403). 대신 **개발 문서는 리포 안(docs/)에 코드와 함께 버전 관리** — 이 역시 표준 관행("docs as code"). Confluence는 회의록/기획 등 코드와 분리된 문서가 생기면 그때 연동 |
...
## 5. Confluence 연동이 필요해지면

이 세션에서 403 "The app is not installed on this instance"가 발생했다.
해결: claude.ai(또는 Claude Code) **커넥터 설정에서 Atlassian 연결에 Confluence 권한(스코프)을 추가**로 승인하거나, Atlassian 관리자 화면에서 해당 앱을 Confluence에도 설치해야 한다. 이후에는 `getConfluenceSpaces` → `createConfluencePage` 흐름으로 페이지 생성 가능.
